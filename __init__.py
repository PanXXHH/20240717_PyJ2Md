...
class PyJ2Md:
    def __init__(self, data: Any = None, env: Environment | None = None):
        self.data = data
    ...
    def render_template(self, template) -> Document.Document:

        doc = Document.Document()

        for extension in self.extensions:
            doc, self.data, template = extension.pre_render_template(
                self.env, doc, self.data, template)

        # 使用环境来渲染模板
        rendered = self.env.from_string(template).render(
