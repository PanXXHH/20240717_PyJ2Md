...
class IN_WRITING(hotconfig.HotConfig):
    ...
    def pre_render_template(self, env: hotconfig.Environment, doc: hotconfig.Document, data: dict, template: str, content: str = "当前处于编辑阶段，内容还在完善中！") -> tuple[hotconfig.Environment, hotconfig.Document, dict, str]:
        if not self.run:
